...
function loadAdminModule(moduleId, event) {
  ...
  if (!contentDiv) {
    ...
    //Variable for API return
    var apiResult = [];

    //Function to call API and load module content
    fetch('api/packages/item_all?sign_key=80b553c62161cc072ce4a8b1abc6e136')
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            const moduleContents = {
                "dashboard": getAdminDashboardContent(apiResult), //Parameterized function to pass API result
            ...
            //Removed function to verify module existence because of error
